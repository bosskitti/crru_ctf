...
with app.app_context():
    ...
    # 1. Clean b_layers_val with flex-direction column (Description box at BOTTOM) and NO raw HTML leaks
    b_layers_val = """### ⚙️ Windows System Architecture Layers

คลิกหรือเลื่อนเมาส์ชี้ไปที่แต่ละกล่ององค์ประกอบในผังโครงสร้างสถาปัตยกรรม Windows ด้านบน เพื่อดูคำอธิบายและหน้าที่การทำงานของส่วนประกอบนั้นๆ ทางด้านล่าง:
...
.w-arch-map{width:100%;display:flex;flex-direction:column;gap:12px;font-family:'JetBrains Mono',monospace;}
.w-mode-sec{border:1px solid;border-radius:8px;padding:14px;position:relative;display:flex;flex-direction:column;gap:8px;}
.w-mode-sec.user-mode{border-color:rgba(0,240,255,0.25);background:rgba(0,240,255,0.015);}
.w-mode-sec.kernel-mode{border-color:rgba(255,0,127,0.25);background:rgba(255,0,127,0.015);}
.w-mode-title-tag{font-size:0.75rem;font-weight:800;text-transform:uppercase;letter-spacing:0.08em;margin-bottom:6px;}
...
.wnode{padding:10px 6px;border-radius:6px;border:1px solid rgba(255,255,255,0.08);background:rgba(255,255,255,0.03);text-align:center;font-size:0.78rem;font-weight:700;color:#94a3b8;cursor:pointer;transition:all 0.18s ease;user-select:none;display:flex;align-items:center;justify-content:center;min-height:36px;box-sizing:border-box;}
.wnode:hover, .wnode.active{color:#ffffff;transform:translateY(-2px);box-shadow:0 0 14px rgba(255,255,255,0.15);background:rgba(255,255,255,0.08);}
.wnode.clr-app{border-color:rgba(61,220,132,0.3);background:rgba(61,220,132,0.04);color:#3ddc84;}
.wnode.clr-app:hover, .wnode.clr-app.active{border-color:#3ddc84;background:rgba(61,220,132,0.18);box-shadow:0 0 14px rgba(61,220,132,0.4);color:#ffffff;}
.wnode.clr-subsys{border-color:rgba(0,240,255,0.3);background:rgba(0,240,255,0.04);color:#00f0ff;}
.wnode.clr-subsys:hover, .wnode.clr-subsys.active{border-color:#00f0ff;background:rgba(0,240,255,0.18);box-shadow:0 0 14px rgba(0,240,255,0.4);color:#ffffff;}
.wnode.clr-ntdll{border-color:rgba(251,191,36,0.3);background:rgba(251,191,36,0.04);color:#fbbf24;}
.wnode.clr-ntdll:hover, .wnode.clr-ntdll.active{border-color:#fbbf24;background:rgba(251,191,36,0.18);box-shadow:0 0 14px rgba(251,191,36,0.4);color:#ffffff;}
.wnode.clr-trap{border-color:rgba(244,114,182,0.3);background:rgba(244,114,182,0.04);color:#f472b6;}
.wnode.clr-trap:hover, .wnode.clr-trap.active{border-color:#f472b6;background:rgba(244,114,182,0.18);box-shadow:0 0 14px rgba(244,114,182,0.4);color:#ffffff;}
.wnode.clr-exec{border-color:rgba(171,32,253,0.3);background:rgba(171,32,253,0.04);color:#c084fc;}
.wnode.clr-exec:hover, .wnode.clr-exec.active{border-color:#ab20fd;background:rgba(171,32,253,0.18);box-shadow:0 0 14px rgba(171,32,253,0.4);color:#ffffff;}
.wnode.clr-hal{border-color:rgba(255,0,127,0.3);background:rgba(255,0,127,0.04);color:#ff007f;}
.wnode.clr-hal:hover, .wnode.clr-hal.active{border-color:#ff007f;background:rgba(255,0,127,0.18);box-shadow:0 0 14px rgba(255,0,127,0.4);color:#ffffff;}
.w-arch-info{width:100%;background:rgba(15,17,26,0.7);border:1px solid rgba(0,240,255,0.25);border-radius:10px;padding:20px;display:flex;flex-direction:column;box-sizing:border-box;}
.w-info-header{display:flex;align-items:center;gap:12px;border-bottom:1px solid rgba(255,255,255,0.08);padding-bottom:12px;margin-bottom:14px;}
.w-info-badge{font-size:0.72rem;font-weight:800;padding:3px 10px;border-radius:20px;text-transform:uppercase;letter-spacing:0.05em;}
.w-info-badge.user{background:rgba(0,240,255,0.12);border:1px solid rgba(0,240,255,0.3);color:#00f0ff;}
.w-info-badge.kernel{background:rgba(255,0,127,0.12);border:1px solid rgba(255,0,127,0.3);color:#ff007f;}
.w-info-title{font-size:1.05rem;font-weight:800;color:#ffffff;margin:0;}
.w-info-body{font-size:0.88rem;color:#cbd5e1;line-height:1.7;}
...
<div class="w-row">
<div class="wnode clr-subsys" style="width:120px;" onclick="showWInfo('subsys-srv', this)" onmouseover="showWInfo('subsys-srv', this)">Subsystem servers</div>
...
<p>โปรแกรมระดับบนสุดที่ผู้ใช้งานเรียกใช้งานโดยตรง เช่น <strong>Chrome, Word, Discord</strong></p>
...
<script>
const winArchData = {
  'apps': { title: 'Applications (โปรแกรมประยุกต์)', badge: 'User Mode', badgeClass: 'user', desc: '<p>โปรแกรมประยุกต์ที่ผู้ใช้งานเรียกเปิดใช้ เช่น <strong>Chrome, Word, Discord</strong></p><p>โปรแกรมในชั้นนี้ทำงานอยู่บนระดับวงแหวนสิทธิ์ Ring 3 เพื่อป้องกันไม่ให้ข้อผิดพลาดของโปรแกรมใดๆ ส่งผลกระทบต่อระบบปฏิบัติการหลัก หากแอปพลิเคชันปิดตัวหรือแครช จะไม่ทำให้ Windows ล่มตามไปด้วย</p>' },
  'subsys-srv': { title: 'Subsystem Servers (เซิร์ฟเวอร์ระบบย่อย)', badge: 'User Mode', badgeClass: 'user', desc: '<p>ควบคุมสภาพแวดล้อมเฉพาะตัวของ Windows เช่น <code>csrss.exe</code> ประสานงานการเปิดใช้งานหน้าต่าง UI การป้อนข้อมูลผ่านเมาส์/คีย์บอร์ด</p>' },
  'dlls': { title: 'DLLs (Dynamic Link Libraries)', badge: 'User Mode', badgeClass: 'user', desc: '<p>เก็บชุดคำสั่งกลางที่เปิดให้โปรแกรมประยุกต์หลายตัวเรียกใช้งานร่วมกัน ช่วยลดการซ้ำซ้อนและประหยัดพื้นที่ RAM</p>' },
  'sys-srv': { title: 'System Services', badge: 'User Mode', badgeClass: 'user', desc: '<p>บริการระบบปฏิบัติการที่ทำงานอยู่ส่วนหลัง (Background Services) เช่น ระบบจัดการคิวเครื่องพิมพ์ Print Spooler</p>' },
  'login-gina': { title: 'Login / GINA', badge: 'User Mode', badgeClass: 'user', desc: '<p>ส่วนยืนยันตัวตนก่อนปล่อยสิทธิ์เข้าสู่หน้า Desktop รับรหัสผ่านหรือลายนิ้วมือเพื่อยืนยันสิทธิ์</p>' },
  'kernel32': { title: 'Kernel32.dll', badge: 'User Mode', badgeClass: 'user', desc: '<p>รับข้อกำหนด API จากโปรแกรมเพื่อควบคุม Process และ Memory นำส่งต่อไปยัง ntdll.dll</p>' },
  'crit-srv': { title: 'Critical Services', badge: 'User Mode', badgeClass: 'user', desc: '<p>บริการสำคัญเช่น <code>lsass.exe</code> ตรวจความปลอดภัยบัญชี หากหยุดทำงาน Windows จะปิดตัวลงทันที</p>' },
  'user32-gdi': { title: 'User32 / GDI', badge: 'User Mode', badgeClass: 'user', desc: '<p>ควบคุมการวาดหน้าต่าง UI กราฟิก เมาส์ และตัวอักษรบนหน้าจอภาพ</p>' },
  'ntdll': { title: 'ntdll.dll (Native API)', badge: 'User Mode', badgeClass: 'user', desc: '<p>สะพานด่านสุดท้ายก่อนข้ามจาก User Mode ไป Kernel Mode แปลงคำสั่งส่งต่อไปยัง Trap Interface</p>' },
  'trap': { title: 'Trap Interface / LPC', badge: 'Kernel Mode', badgeClass: 'kernel', desc: '<p>ประตูด่านตรวจรับ System Calls สลับสิทธิ์เข้าสู่โหมดเคอร์เนล (Ring 0)</p>' },
  'sec-ref': { title: 'Security Reference Monitor (SRM)', badge: 'Kernel Mode', badgeClass: 'kernel', desc: '<p>ตรวจสอบ Token สิทธิ์ความปลอดภัยและการเข้าถึงไฟล์ตามกฎ NTFS Permissions</p>' },
  ...
  'mem-mgr': { title: 'Memory Manager', badge: 'Kernel Mode', badgeClass: 'kernel', desc: '<p>จัดสรรพื้นที่ RAM กายภาพและ Virtual Memory ให้กับระบบ</p>' },
  ...
  'win32-gui': { title: 'Win32k.sys (Kernel GUI)', badge: 'Kernel Mode', badgeClass: 'kernel', desc: '<p>ส่วนเคอร์เนลสำหรับประมวลผลกราฟิกและ Driver จอภาพระดับล่าง</p>' },
...
<div class="mini-quiz-card">
  ...
  <div style="background: rgba(15, 23, 42, 0.6); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 8px; padding: 14px 18px;">
    ...
    db.session.commit()
    print("SUCCESSFULLY APPLIED BOTTOM DESCRIPTION LAYOUT AND FIXED RAW HTML IN LESSON 168!")
