...
# Helper function to generate Interactive Mini-Quiz HTML/JS with Neon Gauge Bar Progress
def make_mini_quiz_html_with_gauge(lesson_num, q1, q1_opts, q1_ans, q2, q2_opts, q2_ans):
    opts1_html = ""
...
function updateMiniProgress(lnum) {{
  ...
  // Set intermediate progress visual before checking correctness
  if (pct > 0) {{
...
function checkMiniQuiz(lnum) {{
  ...
  if (score === 2) {{
    // 100% completed and correct -> Glow green progress
    ...
    // Not passed -> Drop progress bar back to error representation
...
for lid, info in quizzes.items():
    ...
    # Locate index of the quiz block and replace it
    replaced = False
    for i, b in enumerate(blocks):
        ...
        if "Quick Quiz" in val or "Practice Questions" in val or "✏️" in val or "❓" in val:
            ...
            replaced = True
            print(f"Replaced mini-quiz block at index {i} in Lesson {lid}")
            break
            
    if not replaced:
        blocks.append({"type": "markdown", "value": quiz_html})
        print(f"Appended new mini-quiz block to Lesson {lid}")
        
    l.content = json.dumps(blocks, ensure_ascii=False)
    db.session.query(TutorialLesson).filter_by(id=lid).update({"content": l.content})
    db.session.commit()

print("All Lessons 1 to 7 successfully updated with Neon Gauge Bar Progress Quizzes!")
ctx.pop()
