...
# The updated HTML replacement code without any backticks (completely flat, 0-indentation)
html_explorer_safe = """<style>
...
{ name: "X11", type: "folder", path: "/etc/X11", desc: "X-window configurations (หน้าต่างกราฟิก) - โฟลเดอร์เก็บรายละเอียดคอนฟิกูเรชันสำหรับการแสดงผลหน้าจอแบบ GUI (X11)" }
]
},
{
name: "home",
...
{ name: ".history", type: "file", path: "/home/wongyos/.history", desc: "Terminal command history (ประวัติคำสั่งคำรัน) - ไฟล์ประวัติที่คอยบันทึกคำสั่งทั้งหมดที่ผู้ใช้เคยรันพิมพ์ผ่านเทอร์มินัลย้อนหลัง" },
...
{ name: "srv", type: "folder", path: "/srv", desc: "Services (ข้อมูลการให้บริการ) - เก็บไฟล์และข้อมูลที่ใช้ในการให้บริการเครือข่าย เช่น ข้อมูลเว็บไซต์ หรือ FTP" },
...
type: "folder",
path: "/var",
desc: "Variable directory (ข้อมูลผันแปรระบบ) - แหล่งเก็บบันทึกข้อมูลระบบที่มีการเคลื่อนไหว เปลี่ยนแปลง และอัปเดตอย่างต่อเนื่อง เช่น ล็อกไฟล์ (Log) หรือระบบคิวเมล",
children: [
...
{ name: "tmp", type: "folder", path: "/var/tmp", desc: "Temporary (ไฟล์ชั่วคราวข้ามบูต) - โฟลเดอร์เก็บไฟล์ชั่วคราวของแอปพลิเคชันระบบที่จะไม่มีการลบข้อมูลไปแม้จะรีสตาร์ทระบบเปิดระบบเครื่องขึ้นใหม่ก็ตาม" },
{
name: "log",
type: "folder",
path: "/var/log",
...
node.children.map(function(child) { return buildTreeHTML(child); }).join('') +
'</div>';
}
var icon = isFolder 
...
document.querySelectorAll('.fe-node-label').forEach(function(n) { n.classList.remove('active'); });
el.classList.add('active');
var node = findNodeByPath(treeData, path);
if (node) {
...
var arrow = el.querySelector('.fe-toggle-arrow i');
...
var panel = document.getElementById('fe-details-panel-el');
if (panel) {
...
panel.innerHTML = '<div class="fe-details-header">Selected ' + (node.type === 'folder' ? 'Directory' : 'File') + '</div>' +
...
db.session.commit()
print("File Explorer script escaping fixed successfully!")
