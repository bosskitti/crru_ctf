...
def migrate_style1(lid, val):
    # This is a Style 1 (straight bar) quiz. We will convert it to Style 2 (circular gauge with Module 2 gradient).
    print(f"Migrating Lesson {lid} (Style 1 -> Style 2 Circular Gauge)")
...
"""
    if css_pattern.search(val):
        ...
        # Fallback: find </style> and insert our CSS before it
        val = val.replace("</style>", new_css + "\n</style>")

    # 2. Update the HTML structure:
    # Wrap questions in mq-layout-container and mq-questions-col
    box_start_pattern = re.compile(rf'<div id="mq-box-{lid}" class="mini-quiz-box">')
    val = box_start_pattern.sub(f'<div id="mq-box-{lid}" class="mini-quiz-box"><div class="mq-layout-container"><div class="mq-questions-col">', val)
    ...
    # Check alternative matches without comments
    ...
    status_bar_str = f'<div id="mq-status-{lid}" class="mq-status-bar"></div>'
    new_gauge_html = f"""<div id="mq-status-{lid}" class="mq-status-bar"></div>
</div>
<div class="mq-gauge-col">
  ...
  if (status) {{
    ...
    # Replace pbar/ptext assignments in solved state (score === 2)
    ...
    # Replace pbar/ptext assignments in failed state
...
def migrate_style2(lid, val):
    # This is a Style 2 (circular gauge) quiz. We will update the stroke color to use the Module 2 color gradient.
    print(f"Migrating Lesson {lid} (Style 2 Circular Gauge Color Scheme)")
    
    # 1. Inject linearGradient inside the SVG
    ...
    # 2. Update .neon-gauge-fill styling in the CSS block to use url(#gauge-grad-{lid})
    ...
    new_gauge_fill_css = f".neon-gauge-fill {{fill:none; stroke:url(#gauge-grad-{lid}); stroke-width:2.8; stroke-linecap:round; transition:stroke-dasharray 0.5s ease, stroke 0.5s ease; filter:drop-shadow(0 0 5px rgba(0,240,255,0.4));}}"
    val = gauge_fill_css_pattern.sub(new_gauge_fill_css, val)

    # 3. Update JS updateLocalProgress function to use fill.style.stroke = '' instead of solid color on reset
    js_update_stroke_pattern = re.compile(rf'if\s*\(fill\)\s*fill\.style\.stroke\s*=\s*[\'\"].*?[\'\"];', re.DOTALL)
    val = val.replace(f"if (fill) fill.style.stroke = '#00f0ff';", "if (fill) fill.style.stroke = '';")
    val = val.replace(f"if (fill) fill.style.stroke = '#a855f7';", "if (fill) fill.style.stroke = '';")
    val = val.replace(f"if (fill) fill.style.stroke = '#10b981';", "if (fill) fill.style.stroke = '';")
    val = val.replace(f"if (fill) fill.style.stroke = '#3b82f6';", "if (fill) fill.style.stroke = '';")
        ...
        try:
            ...
            for idx, b in enumerate(blocks):
                ...
                if 'Lesson Quick Quiz' in val:
                    # Determine style
                    if 'neon-gauge' in val:
                        # Style 2
                        new_val = migrate_style2(l.id, val)
                        b['value'] = new_val
                        modified = True
                    elif 'mq-progress-bar' in val:
                        # Style 1
                        new_val = migrate_style1(l.id, val)
                        b['value'] = new_val
                        modified = True
            ...
            if modified:
                ...
                print(f"Successfully migrated quiz for Lesson ID {l.id}!")
