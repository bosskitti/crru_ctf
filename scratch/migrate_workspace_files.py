...
def migrate_style1_content(val, lnum):
    # css
...
<div class="mq-gauge-col">
  ...
  if (status) {{
    ...
    # js checkMiniQuiz
    pbar_def_pattern = re.compile(r'const pbar = document\.getElementById\(\'mq-progress-bar-\' \+ lnum\);|var pbar = document\.getElementById\(\'mq-progress-bar-\' \+ lnum\);')
    ptext_def_pattern = re.compile(r'const ptext = document\.getElementById\(\'mq-progress-text-\' \+ lnum\);|var ptext = document\.getElementById\(\'mq-progress-text-\' \+ lnum\);')
...
def migrate_style2_content(val, lnum):
    #defs
    svg_pattern = re.compile(r'<svg class="neon-gauge" viewBox="0 0 36 36">')
    grad_defs = f"""<svg class="neon-gauge" viewBox="0 0 36 36">
      <defs>
        <linearGradient id="gauge-grad-{lnum}" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stop-color="#ff007f" />
          <stop offset="50%" stop-color="#fbbf24" />
    ...
    val = svg_pattern.sub(grad_defs, val)

    #css
    gauge_fill_css_pattern = re.compile(r'\.neon-gauge-fill\s*\{fill:none;\s*stroke:[^;]+;\s*stroke-width:2\.8;\s*stroke-linecap:round;\s*transition:[^;]+;\s*filter:[^;]+\}')
    new_gauge_fill_css = f".neon-gauge-fill {{fill:none; stroke:url(#gauge-grad-{lnum}); stroke-width:2.8; stroke-linecap:round; transition:stroke-dasharray 0.5s ease, stroke 0.5s ease; filter:drop-shadow(0 0 5px rgba(0,240,255,0.4));}}"
    val = gauge_fill_css_pattern.sub(new_gauge_fill_css, val)

    #js reset
    val = val.replace("if (fill) fill.style.stroke = '#00f0ff';", "if (fill) fill.style.stroke = '';")
    val = val.replace("if (fill) fill.style.stroke = '#a855f7';", "if (fill) fill.style.stroke = '';")
    val = val.replace("if (fill) fill.style.stroke = '#10b981';", "if (fill) fill.style.stroke = '';")
    val = val.replace("if (fill) fill.style.stroke = '#3b82f6';", "if (fill) fill.style.stroke = '';")
...
def migrate_file(filepath):
    ...
    # We need to find block content strings that represent quizzes and migrate them
    # For create_w198_networking.py, etc., it has lnum 343 for networking, 345 for crypto, 346 for RE, 347 for exploit
    # Let's see: we can parse the lesson ID from the file content or path
    lnum = 1
    if '198' in filepath:
        lnum = 343
    elif '199' in filepath:
        lnum = 345
    elif '200' in filepath:
        lnum = 346
    elif '201' in filepath:
        lnum = 347

    # Let's search for "### ✏️ Lesson Quick Quiz"
    if '### ✏️ Lesson Quick Quiz' in content:
        # Find where it starts and ends
        # In files, it is usually inside triple quotes
        # We can find the triple-quoted string that contains the quiz
        # E.g., """### ✏️ Lesson Quick Quiz..."""
        pattern = re.compile(r'(""\"|\'\'\')### ✏️ Lesson Quick Quiz.*?\1', re.DOTALL)
        matches = list(pattern.finditer(content))
        for m in matches:
            quiz_str = m.group(0)
            quote = m.group(1)
            # Remove quotes for migration
            raw_quiz = quiz_str[3:-3]
            
            # Decide style
            if 'neon-gauge' in raw_quiz:
                # We need to extract lesson number from context or use the file-specific lnum
                # For fix_final_v7.py or term_w36, let's extract the number from the variable name or content, e.g. lessonKey177
                lid_match = re.search(r'lessonKey(\d+)|solved_lesson_(\d+)', raw_quiz)
                current_lnum = lnum
                if lid_match:
                    current_lnum = int(lid_match.group(1) or lid_match.group(2))
                new_raw = migrate_style2_content(raw_quiz, current_lnum)
            else:
                # Style 1 straight bar
                lid_match = re.search(r'mq-box-(\d+)|mq-progress-bar-(\d+)', raw_quiz)
                current_lnum = lnum
                if lid_match:
                    current_lnum = int(lid_match.group(1) or lid_match.group(2))
                new_raw = migrate_style1_content(raw_quiz, current_lnum)

            new_quiz_str = quote + new_raw + quote
            content = content.replace(quiz_str, new_quiz_str)

    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"Successfully migrated file: {filepath}")

# List of files in workspace to migrate
files_to_migrate = [
    '/home/kali/crru_ctf/CTFd/create_w198_networking.py',
    '/home/kali/crru_ctf/CTFd/create_w199_crypto.py',
    '/home/kali/crru_ctf/CTFd/create_w200_re.py',
    '/home/kali/crru_ctf/CTFd/create_w201_exploit.py',
