...
**คุกกี้ (HTTP Cookies / Web Cookies)** คือข้อมูลชิ้นเล็กๆ ที่เว็บเซิร์ฟเวอร์สร้างขึ้นและจัดส่งมาเก็บันทึกไว้ในเครื่องผู้ใช้งาน (Client-side) ผ่านเว็บบราวเซอร์ คุกกี้จะถูกแนบส่งกลับไปหาเซิร์ฟเวอร์ในทุกการเข้าถึงหน้าเว็บของโดเมนนั้นเพื่อยืนยันตัวตนเซสชัน (Session Tracking) และจดจำการตั้งค่าผู้ใช้
...
with app.app_context():
    ...
    if l:
        try:
            ...
            # Find the specific target marker index
            target_marker = "### 🛠️ เครื่องมือแกะรหัสหน้าเว็บ (Web Page Source Inspections)"
            idx = val.find(target_marker)
            
            if idx != -1:
                # Replace ONLY from the target marker to the end of value
                new_val = val[:idx] + expanded_html_section
                blocks[0]["value"] = new_val
                l.content = json.dumps(blocks, ensure_ascii=False)
                app.db.session.commit()
                print("Successfully expanded ONLY the Source Inspections section, keeping the rest of the block untouched!")
            else:
                print("Error: Could not find target marker in database value.")
