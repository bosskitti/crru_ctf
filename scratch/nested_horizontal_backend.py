...
# ─── Rewrite Block 1 using a 1fr 2fr 1fr Grid, placing backend internals horizontally inside the middle card ───
...
.web-arch-grid{display:grid;grid-template-columns:1fr 2fr 1fr;gap:16px;margin:2rem auto 1rem;max-width:1050px;}
...
/* Horizontal layout inside the middle Web Back-End card */
...
<!-- 3-tier Grid with horizontal nested layout in Back-End -->
<div class="web-arch-grid">
<!-- Front-End (Left) -->
<div class="web-arch-card">
<div>
<div class="web-arch-title">🌐 Web Front-End (Client)</div>
<p class="web-arch-desc">ส่วนนำเสนอผลบนบราวเซอร์ฝั่งผู้ใช้ รันและจัดโครงสร้างด้วยภาษา <strong>HTML, CSS และ JavaScript</strong> เพื่อสร้างและวาดหน้าจอรับส่งข้อมูล</p>
</div>
</div>

<!-- Back-End (Middle - 2x Wide with 3 internal cards lined up horizontally) -->
<div class="web-arch-card" style="border-color:rgba(0,240,255,0.15);">
<div>
<div class="web-arch-title" style="color:#ffffff;">⚙️ Web Back-End (Server-Side internals)</div>
...
<p class="web-arch-desc">ส่วนจัดเก็บข้อมูลระบบระดับเซิร์ฟเวอร์ฐานข้อมูลด้วยภาษา <strong>SQL</strong> คอยประมวลผล Query ข้อมูลผ่านสคริปต์ส่งคำสั่งหลังบ้าน</p>
</div>
</div>
</div>
...
db.session.commit()
print("Web Back-End internals successfully integrated horizontally inside the main middle card!")
ctx.pop()
