...
def make_style1_quiz_html(lid, q1_title, q1_opts, q1_correct, q2_title, q2_opts, q2_correct, filepath):
    ...
    # Handle javascript backslashes escaping inside multi-line python string
    double_slash = "\\" if "term_w36" in filepath else ""
...
def patch_file(filepath):
    ...
    lessons_data = {
        ...
        180: {
            "q1_title": "ส่วนหัว (Header) ความปลอดภัยใดใน HTTP Response ที่ใช้ป้องกันหน้าเว็บไม่ให้ถูกนำไปฝังใน Iframe เพื่อเลี่ยงช่องโหว่ Clickjacking?",
    ...
    for lid, d in lessons_data.items():
        var_name = f"{prefix}_{lid}" if prefix == "QUIZ" else f"val{lid}_2"
        ...
        new_quiz_html = make_style1_quiz_html(
            lid,
        ...
        # Replace the full variable declaration
        r_prefix = "r" if prefix == "QUIZ" else ""
        old_full = var_name + " = " + r_prefix + quote + content[start_pos + 3 : end_pos] + quote
        new_full = var_name + " = " + r_prefix + quote + new_quiz_html + quote
        
        content = content.replace(old_full, new_full)
        modified = True
        print(f"Patched Lesson {lid} to Style 1 Layout in workspace file.")
