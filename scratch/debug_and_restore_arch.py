...
# Let's find the block that contains Linux System Architecture
target_idx = None
for idx, b in enumerate(blocks):
    val = b.get('value', '')
    if "Linux System Architecture" in val and "arch-container" in val:
        target_idx = idx
        print(f"Found target architecture block at index {idx}")
        break
...
if target_idx is not None:
    blocks[target_idx]['value'] = html_arch_blueprint
    lesson.content = json.dumps(blocks, ensure_ascii=False)
    db.session.query(TutorialLesson).filter_by(id=165).update({"content": lesson.content})
    db.session.commit()
    print("Success: Linux System Architecture block updated with side-by-side explanations!")
else:
    print("Error: Could not find architecture block in lesson content!")
    # Let's inspect block 2 manually to see what's in there
    if len(blocks) > 2:
        print("Block 2 content snippet:", blocks[2]['value'][:150])
