...
# Pure HTML/SVG block with absolutely ZERO leading indentation spaces on any line.
...
with app.app_context():
    ...
    if l:
        try:
            ...
            # Find the enclosing container of our SVG block (starts with <div style="background:#070a13; border: or similar)
            target_start = val.find('<div style="background:#070a13;')
            if target_start == -1:
                target_start = val.find('<div style="background:#070a13;')
                
            target_end = val.find('</div>', target_start)
            if target_end != -1:
                target_end += 6
                
            if target_start != -1 and target_end != -1:
                # Replace with the unindented SVG block
                ...
                print("Successfully updated SVG diagram to be completely unindented!")
            else:
                print("Error: Could not find target SVG block container in Lesson 177 Block 0.")
