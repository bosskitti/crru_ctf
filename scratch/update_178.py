...
.w-sandbox-nav:hover, .w-nav-item.active { border-color: #a855f7 !important; color: #ffffff !important; background: rgba(168, 85, 247, 0.05) !important; }
...
<button id="nav-item-dirb" class="w-nav-item active">1. Dirb Backup Scan</button>
<button id="nav-item-curl" class="w-nav-item">2. cURL HTTP Audit</button>
</div>
...
<button id="btn-run-dirb" style="background: rgba(168,85,247,0.1); border: 1px solid rgba(168,85,247,0.3); border-radius: 4px; color: #a855f7; font-size: 0.68rem; padding: 3px 8px; cursor: pointer; font-family: monospace; font-weight: 700; transition: all 0.15s;">▶ Run Simulation</button>
</div>
...
<div style="background: rgba(255,255,255,0.03); padding: 6px 12px; border-bottom: 1px solid rgba(255,255,255,0.05); display: flex; justify-content: space-between; align-items: center;">
<span style="font-size: 0.65rem; color: #64748b; font-weight: 800; letter-spacing: 0.06em; font-family: monospace;">🐚 Terminal Console</span>
<button id="btn-run-curl" style="background: rgba(168,85,247,0.1); border: 1px solid rgba(168,85,247,0.3); border-radius: 4px; color: #a855f7; font-size: 0.68rem; padding: 3px 8px; cursor: pointer; font-family: monospace; font-weight: 700; transition: all 0.15s;">▶ Run Simulation</button>
</div>
...
document.addEventListener('click', function(e) {
  const tabDirb = e.target.closest('#nav-item-dirb');
  if (tabDirb) {
    const tabCurl = document.getElementById('nav-item-curl');
    const panelDirb = document.getElementById('panel-dirb');
    const panelCurl = document.getElementById('panel-curl');
    if (panelDirb && panelCurl) {
      panelDirb.style.display = 'block';
      panelCurl.style.display = 'none';
      tabDirb.classList.add('active');
      if (tabCurl) tabCurl.classList.remove('active');
    }
    return;
  }

  const tabCurl = e.target.closest('#nav-item-curl');
  if (tabCurl) {
    const tabDirb = document.getElementById('nav-item-dirb');
    const panelDirb = document.getElementById('panel-dirb');
    const panelCurl = document.getElementById('panel-curl');
    if (panelDirb && panelCurl) {
      panelDirb.style.display = 'none';
      panelCurl.style.display = 'block';
      tabCurl.classList.add('active');
      if (tabDirb) tabDirb.classList.remove('active');
    }
    return;
  }

  const runDirb = e.target.closest('#btn-run-dirb');
  if (runDirb) {
    window.startWebSim('dirb');
    return;
  }

  const runCurl = e.target.closest('#btn-run-curl');
  if (runCurl) {
    window.startWebSim('curl');
    return;
  }
});
</script>"""
