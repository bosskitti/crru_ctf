...
new_http_understanding_html = """### 🌐 ความเข้าใจเกี่ยวกับข้อความสื่อสาร HTTP (HTTP Message Understanding)
...
### 🚦 รหัสสถานะตอบรับการสื่อสาร (HTTP Status Codes Overview)

เว็บเซิร์ฟเวอร์จะตอบกลับไคลเอนต์ด้วย **รหัสสถานะความยาว 3 หลัก** เสมอ เพื่อรายงานให้ทราบว่าเกิดอะไรขึ้นกับคำขอดังกล่าว โดยสรุปหมวดหมู่หลักระดับกว้างได้ดังนี้:

<div style="overflow-x:auto;margin:1.5rem auto;max-width:1000px;border:1px solid rgba(234,179,8,0.25);border-radius:12px;background:#05070f;box-shadow:0 10px 30px rgba(0,0,0,0.6);">
<table style="width:100%;border-collapse:collapse;text-align:left;font-family:sans-serif;font-size:0.83rem;color:#cbd5e1;">
<thead>
<tr style="background:rgba(234,179,8,0.08);border-bottom:1px solid rgba(234,179,8,0.2);">
<th style="padding:12px 16px;font-weight:bold;color:#eab308;width:15%;">รหัสสถานะ (Code)</th>
<th style="padding:12px 16px;font-weight:bold;color:#ffffff;width:30%;">ความหมายหลัก (Meaning)</th>
<th style="padding:12px 16px;font-weight:bold;color:#3ddc84;width:55%;">ตัวอย่างรหัสที่พบบ่อย (Examples)</th>
</tr>
</thead>
<tbody>
<tr style="border-bottom:1px solid rgba(255,255,255,0.04);background:rgba(255,255,255,0.005);">
<td style="padding:12px 16px;color:#94a3b8;font-weight:bold;">1xx</td>
<td style="padding:12px 16px;color:#94a3b8;">Informational (แจ้งข้อมูลเบื้องต้น)</td>
<td style="padding:12px 16px;"><code style="color:#94a3b8;">100 Continue</code> (ได้รับคำขอแล้ว ให้ส่งข้อมูลต่อ)</td>
</tr>
<tr style="border-bottom:1px solid rgba(255,255,255,0.04);background:rgba(255,255,255,0.015);">
<td style="padding:12px 16px;color:#3ddc84;font-weight:bold;">2xx</td>
<td style="padding:12px 16px;color:#3ddc84;">Success (ทำงานสำเร็จสมบูรณ์)</td>
<td style="padding:12px 16px;"><code style="color:#3ddc84;font-weight:bold;">200 OK</code> (ประมวลผลสำเร็จ), <code style="color:#3ddc84;">201 Created</code> (สร้างข้อมูลสำเร็จ)</td>
</tr>
<tr style="border-bottom:1px solid rgba(255,255,255,0.04);background:rgba(255,255,255,0.005);">
<td style="padding:12px 16px;color:#3b82f6;font-weight:bold;">3xx</td>
<td style="padding:12px 16px;color:#3b82f6;">Redirection (ส่งตัวผู้ใช้ไปพาธอื่น)</td>
<td style="padding:12px 16px;"><code style="color:#60a5fa;">301 Moved Permanently</code> (ย้ายที่อยู่ถาวร), <code style="color:#60a5fa;">302 Found</code> (ย้ายชั่วคราว)</td>
</tr>
<tr style="border-bottom:1px solid rgba(255,255,255,0.04);background:rgba(255,255,255,0.015);">
<td style="padding:12px 16px;color:#f59e0b;font-weight:bold;">4xx</td>
<td style="padding:12px 16px;color:#f59e0b;">Client Error (ข้อผิดพลาดที่ฝั่งผู้เล่น/ไคลเอนต์)</td>
<td style="padding:12px 16px;"><code style="color:#f59e0b;">400 Bad Request</code>, <code style="color:#f59e0b;font-weight:bold;">401 Unauthorized</code> (ไม่มีสิทธิ์), <code style="color:#f59e0b;font-weight:bold;">403 Forbidden</code> (ห้ามเข้า), <code style="color:#f59e0b;font-weight:bold;">404 Not Found</code> (ไม่พบพาธ)</td>
</tr>
<tr style="border-bottom:1px solid rgba(255,255,255,0.04);background:rgba(255,255,255,0.005);">
<td style="padding:12px 16px;color:#ef4444;font-weight:bold;">5xx</td>
<td style="padding:12px 16px;color:#ef4444;">Server Error (ข้อผิดพลาดจากฝั่งเครื่องเซิร์ฟเวอร์)</td>
<td style="padding:12px 16px;"><code style="color:#ef4444;font-weight:bold;">500 Internal Server Error</code> (เซิร์ฟเวอร์ขัดข้อง/ล่ม), <code style="color:#ef4444;">503 Service Unavailable</code></td>
</tr>
</tbody>
</table>
</div>
...
> [!NOTE]
> **การวิเคราะห์รายละเอียดเชิงลึกของแต่ละ HTTP Status Codes**:
> ตารางด้านบนคือกลุ่มสถานะ HTTP แบบสรุปย่อ แอดมินและผู้เรียนสามารถคลิกข้ามไปอ่านแบบจำลองวิเคราะห์อย่างละเอียด (เช่น การโจมตีและความหมายเชิงลึกในการเจาะระบบ) ได้ที่หัวข้อนี้ด้านล่างทันทีครับ: [🔗 คู่มือวิเคราะห์ HTTP Status Codes และการตรวจสอบสิทธิ์ความปลอดภัย](#http-status-codes-details)
...
with app.app_context():
    ...
    if l:
        try:
            ...
            # 1. Insert explicit anchor target div before the detailed status codes header
            # The header is "#### 💡 การทำความเข้าใจ HTTP Status Codes จากผลลัพธ์ของ DIRB"
            target_hdr = "#### 💡 การทำความเข้าใจ HTTP Status Codes จากผลลัพธ์ของ DIRB"
            hdr_idx = val.find(target_hdr)
            
            if hdr_idx != -1:
                # Add <div id="http-status-codes-details"></div> right before it
                val = val[:hdr_idx] + '<div id="http-status-codes-details"></div>\n' + val[hdr_idx:]
                print("Successfully added HTML target anchor div for detailed HTTP status codes!")
            else:
                print("Warning: Detailed status codes header not found in value.")
                
            # 2. Overwrite the old "HTTP Message Understanding" section with the new one containing the rough table & updated link
            # Find the old section start
            start_marker = "### 🌐 ความเข้าใจเกี่ยวกับข้อความสื่อสาร HTTP"
            ...
            if start_idx != -1 and end_idx != -1:
                ...
                new_val = val[:start_idx] + new_http_understanding_html + "\n\n" + val[end_idx:]
                blocks[0]["value"] = new_val
                l.content = json.dumps(blocks, ensure_ascii=False)
                app.db.session.commit()
                print("Successfully updated HTTP Message Understanding with rough table and fixed link!")
            else:
                print("Error: Could not find target section boundaries to update.")
