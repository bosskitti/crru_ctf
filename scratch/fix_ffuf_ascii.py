...
with app.app_context():
    ...
    if l:
        try:
            ...
            # Find the pre block with the broken ASCII art
            ...
            if idx != -1 and end_idx != -1:
                # Replace it with our HTML-entity encoded version
                new_val = val[:idx] + new_ascii_html + val[end_idx:]
                blocks[0]["value"] = new_val
                l.content = json.dumps(blocks, ensure_ascii=False)
                app.db.session.commit()
                print("Successfully updated FFUF ASCII logo with HTML character entity codes (&#92;)!")
            else:
