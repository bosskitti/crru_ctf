...
# ─── Rewrite Block 3 of Lesson 172 with HTML Entities fixed and Live Flow Terminal Simulator ───
blocks[3]['value'] = """### 💻 Code Security Console (วิเคราะห์เจาะลึก 8 รูปแบบบั๊กและตรรกะระบบ)

คลิกหัวข้อด้านซ้ายมือเพื่อตรวจสอบซอร์สโค้ด และ **กดปุ่มรันจำลองการโจมตีจริง (Run Simulation)** เพื่อดูผลลัพธ์ผ่านเทอร์มินัลระบบ:
...
/* Terminal simulation console box */
...
<button id="nav-item-intoverflow" class="w-nav-item active" onclick="showSandboxItem('intoverflow', this)">1. Integer Overflow</button>
<button id="nav-item-floatprecision" class="w-nav-item" onclick="showSandboxItem('floatprecision', this)">2. Floating Point Precision</button>
<button id="nav-item-arithside" class="w-nav-item" onclick="showSandboxItem('arithside', this)">3. Arithmetic Side Effects</button>
<button id="nav-item-autoboxing" class="w-nav-item" onclick="showSandboxItem('autoboxing', this)">4. Autoboxing Problems</button>
<button id="nav-item-strop" class="w-nav-item" onclick="showSandboxItem('strop', this)">5. String Operation Issues</button>
<button id="nav-item-formatstr" class="w-nav-item" onclick="showSandboxItem('formatstr', this)">6. Format String Vulnerabilities</button>
<button id="nav-item-bufoverflow" class="w-nav-item" onclick="showSandboxItem('bufoverflow', this)">7. Buffer Overflow Attacks</button>
<button id="nav-item-rngattack" class="w-nav-item" onclick="showSandboxItem('rngattack', this)">8. RNG Predictability</button>
</div>
...
<!-- 1. Integer Overflow -->
<div id="panel-intoverflow" class="w-sand-panel">
...
<button class="w-sand-term-btn" onclick="startTerminalSim('intoverflow')">▶ Run Simulation</button>
</div>
<div id="term-intoverflow" class="w-sand-term"><span class="prompt">kali@kali:~/Desktop$</span> [กดปุ่ม Run Simulation ด้านขวาบนเพื่อทำการจำลอง Flow การโจมตี]</div>
</div>
<div class="w-sand-expl">
<h5>🔒 Security & Code Analysis</h5>
<p>ความเสียหายของตัวแปรประเภทจำนวนเต็มเมื่อรันคณิตศาสตร์เกินขีดจำกัด:</p>
<ul>
<li><strong>Wrap-around</strong>: ตัวแปร 32-bit signed integer มีค่าสูงที่สุดเท่ากับ <code>2147483647</code> เมื่อบวก 1 ผลลัพธ์จะหมุนกลับมาเป็นค่าต่ำสุดคือ <code>-2147483648</code></li>
<li><strong>Security Exploit</strong>: แฮกเกอร์สามารถส่งค่าระดับสูงเพื่อสั่งลบเงื่อนไขเช็คค่าบวกของระบบ (เช่น <code>x &lt; 0</code>) เพื่อเลี่ยงการตรวจเช็คในด่านล็อกอินได้</li>
</ul>
</div>
</div>
...
<div class="w-sand-term-bar">
<span class="w-sand-term-title">🐚 Terminal Console</span>
<button class="w-sand-term-btn" onclick="startTerminalSim('floatprecision')">▶ Run Simulation</button>
</div>
<div id="term-floatprecision" class="w-sand-term"><span class="prompt">kali@kali:~/Desktop$</span> [กดปุ่ม Run Simulation ด้านขวาบนเพื่อทำการจำลอง Flow การโจมตี]</div>
</div>
<div class="w-sand-expl">
<h5>🔒 Security & Code Analysis</h5>
<p>ข้อจำกัดของระบบการจดจำหมายเลขทศนิยมในสถาปัตยกรรม IEEE 754:</p>
<ul>
<li><strong>IEEE 754 Binary Representation</strong>: คอมพิวเตอร์เก็บข้อมูลทศนิยมในรูปของเลขฐานสอง ทำให้ค่าเช่น 0.1 หรือ 0.2 มีค่าเบี่ยงเบนเล็กน้อย (เช่น 0.1 + 0.2 = 0.30000000000000004) และส่งผลให้สมการเช็คเท่ากับ (<code>==</code>) ล้มเหลว</li>
</ul>
</div>
</div>
...
<div class="w-sand-term-bar">
<span class="w-sand-term-title">🐚 Terminal Console</span>
<button class="w-sand-term-btn" onclick="startTerminalSim('arithside')">▶ Run Simulation</button>
</div>
<div id="term-arithside" class="w-sand-term"><span class="prompt">kali@kali:~/Desktop$</span> [กดปุ่ม Run Simulation ด้านขวาบนเพื่อทำการจำลอง Flow การโจมตี]</div>
</div>
<div class="w-sand-expl">
<h5>🔒 Security & Code Analysis</h5>
<p>ความคลาดเคลื่อนระดับบิตและพฤติกรรมที่ไม่พึงประสงค์จากการคำนวณ:</p>
<ul>
<li><strong>NaN (Not a Number)</strong>: ค่าเปรียบเทียบพิเศษของทศนิยม ซึ่งมีลักษณะเฉพาะตัวคือ <code>NaN != NaN</code> แฮกเกอร์สามารถใช้คุณสมบัตินี้ป้อนค่า NaN เพื่อเลี่ยงตรรกะระบบเช็คตัวแปรได้</li>
</ul>
</div>
</div>
...
<div class="w-sand-term-bar">
<span class="w-sand-term-title">🐚 Terminal Console</span>
<button class="w-sand-term-btn" onclick="startTerminalSim('autoboxing')">▶ Run Simulation</button>
</div>
<div id="term-autoboxing" class="w-sand-term"><span class="prompt">kali@kali:~/Desktop$</span> [กดปุ่ม Run Simulation ด้านขวาบนเพื่อทำการจำลอง Flow การโจมตี]</div>
</div>
<div class="w-sand-expl">
<h5>🔒 Security & Code Analysis</h5>
<p>ความเสียหายจากระบบแปลงตัวแปรอัตโนมัติระหว่างคลาสออบเจกต์และตัวแปรพื้นฐานใน Java:</p>
<ul>
<li><strong>Integer Cache Limit</strong>: Java จะแคชออบเจกต์ของตัวเลขอัตโนมัติเฉพาะในช่วง <code>-128</code> ถึง <code>127</code> หากตัวแปรมีค่า 128 ขึ้นไป ตัวเปรียบเทียบ <code>==</code> จะเป็นการเช็คตรรกะพิกัดออบเจกต์ (Reference Address) แทนการเทียบค่าข้างใน</li>
...
<div class="w-sand-term-bar">
<span class="w-sand-term-title">🐚 Terminal Console</span>
<button class="w-sand-term-btn" onclick="startTerminalSim('strop')">▶ Run Simulation</button>
</div>
<div id="term-strop" class="w-sand-term"><span class="prompt">kali@kali:~/Desktop$</span> [กดปุ่ม Run Simulation ด้านขวาบนเพื่อทำการจำลอง Flow การโจมตี]</div>
</div>
<div class="w-sand-expl">
<h5>🔒 Security & Code Analysis</h5>
...
<div class="w-sand-term-bar">
<span class="w-sand-term-title">🐚 Terminal Console</span>
<button class="w-sand-term-btn" onclick="startTerminalSim('formatstr')">▶ Run Simulation</button>
</div>
<div id="term-formatstr" class="w-sand-term"><span class="prompt">kali@kali:~/Desktop$</span> [กดปุ่ม Run Simulation ด้านขวาบนเพื่อทำการจำลอง Flow การโจมตี]</div>
</div>
<div class="w-sand-expl">
<h5>🔒 Security & Code Analysis</h5>
<p>ความเสียหายจากการวางตำแหน่งตัวแปรให้อินพุตควบคุมฟังก์ชันแสดงผลฟอร์แมตโดยตรง:</p>
<ul>
<li><strong>Format Specifier Dump</strong>: ใน C ฟังก์ชัน <code>printf(user_input)</code> เปิดโอกาสให้แฮกเกอร์ป้อนรหัสฟอร์แมตระบุตำแหน่ง เช่น <code>%x</code> หรือ <code>%p</code> เพื่อดึงข้อมูลสถานะในหน่วยความจำ Stack ออกมาทั้งหมด (Dump) หรือป้อนค่า <code>%n</code> เขียนทับตำแหน่งของสิทธิ์แอปพลิเคชัน</li>
</ul>
</div>
</div>
...
<div class="w-sand-term-bar">
<span class="w-sand-term-title">🐚 Terminal Console</span>
<button class="w-sand-term-btn" onclick="startTerminalSim('bufoverflow')">▶ Run Simulation</button>
</div>
<div id="term-bufoverflow" class="w-sand-term"><span class="prompt">kali@kali:~/Desktop$</span> [กดปุ่ม Run Simulation ด้านขวาบนเพื่อทำการจำลอง Flow การโจมตี]</div>
</div>
<div class="w-sand-expl">
<h5>🔒 Security & Code Analysis</h5>
...
<div class="w-sand-term-bar">
<span class="w-sand-term-title">🐚 Terminal Console</span>
<button class="w-sand-term-btn" onclick="startTerminalSim('rngattack')">▶ Run Simulation</button>
</div>
<div id="term-rngattack" class="w-sand-term"><span class="prompt">kali@kali:~/Desktop$</span> [กดปุ่ม Run Simulation ด้านขวาบนเพื่อทำการจำลอง Flow การโจมตี]</div>
</div>
<div class="w-sand-expl">
<h5>🔒 Security & Code Analysis</h5>
...
<script>
// Tab Swapper logic (with displays guaranteed)
...
// Ensure the first active tab is rendered correctly upon execution
setTimeout(() => {
  const activeBtn = document.querySelector('.w-sandbox-nav .w-nav-item.active');
  if (activeBtn) { activeBtn.click(); }
}, 100);

// Live Terminal Flow Simulation logic
window.startTerminalSim = function(itemKey) {
  const term = document.getElementById('term-' + itemKey);
  if (!term) return;

  term.innerHTML = '<span class="prompt">kali@kali:~/Desktop$</span> <span class="cmd">Initializing exploit flow...</span>\\n[.] Connecting resources\\n[.] Running target code...';

  setTimeout(() => {
    if (itemKey === 'intoverflow') {
      term.innerHTML = '<span class="prompt">kali@kali:~/Desktop$</span> <span class="cmd">./integer_overflow_test</span>\\nBefore: 2147483647\\nAfter: -2147483648 (wrap-around)\\n\\n<span class="prompt">kali@kali:~/Desktop$</span> <span class="cmd">java TestJava</span>\\nEnter x: 2147483647\\n<span style="color:#3ddc84; font-weight:bold;">You Win</span>';
    } else if (itemKey === 'floatprecision') {
      term.innerHTML = '<span class="prompt">kali@kali:~/Desktop$</span> <span class="cmd">python3 float_test.py</span>\\n<span style="color:#ff007f;">Not Equal</span>\\n\\n<span class="prompt">kali@kali:~/Desktop$</span> <span class="cmd">./precision_loss_c</span>\\nResult: 10000000000000000 (1.0 is ignored!)';
    } else if (itemKey === 'arithside') {
      term.innerHTML = '<span class="prompt">kali@kali:~/Desktop$</span> <span class="cmd">python3 nan_test.py</span>\\n<span style="color:#3ddc84; font-weight:bold;">You Win (NaN comparison bypassed)</span>\\n\\n<span class="prompt">kali@kali:~/Desktop$</span> <span class="cmd">python3 -c "print(10 / 0)"</span>\\n<span style="color:#ff007f;">ZeroDivisionError: division by zero</span>';
    } else if (itemKey === 'autoboxing') {
      term.innerHTML = '<span class="prompt">kali@kali:~/Desktop$</span> <span class="cmd">java TestJavaCache</span>\\na == b: true\\nc == d: false\\n\\n<span class="prompt">kali@kali:~/Desktop$</span> <span class="cmd">java TestJavaBypass</span>\\nEnter x: 128\\nEnter y: 128\\n<span style="color:#3ddc84; font-weight:bold;">You Win (Object reference mismatch!)</span>';
    } else if (itemKey === 'strop') {
      term.innerHTML = '<span class="prompt">kali@kali:~/Desktop$</span> <span class="cmd">python3 replacement_test.py</span>\\nEnter s: &lt;?&lt;?phpphp\\nFilter Output: &lt;?php\\n<span style="color:#3ddc84; font-weight:bold;">You Win (Payload evasion success)</span>';
    } else if (itemKey === 'formatstr') {
      term.innerHTML = '<span class="prompt">kali@kali:~/Desktop$</span> <span class="cmd">./format_string_test</span>\\nGuess: %x %x %x %x\\n<span style="color:#fbbf24; font-weight:bold;">ffe012ac 0 4012ba4d ffe012c4 (Stack Memory Dumped!)</span>\\nYou Lose';
    } else if (itemKey === 'bufoverflow') {
      term.innerHTML = '<span class="prompt">kali@kali:~/Desktop$</span> <span class="cmd">python3 -c "print(\\'A\\'*28 + \\'\\\\x39\\\\x30\\\\x00\\\\x00\\')" | ./buffer_overflow</span>\\nEnter Your Name: \\n<span style="color:#3ddc84; font-weight:bold;">You Win (Variable a overwritten to 12345!)</span>';
    } else if (itemKey === 'rngattack') {
      term.innerHTML = '<span class="prompt">kali@kali:~/Desktop$</span> <span class="cmd">python3 Test.py</span>\\n54 94 2 39 48 25 35 73 56 21\\n\\n<span class="prompt">kali@kali:~/Desktop$</span> <span class="cmd">python3 Test.py</span>\\n54 94 2 39 48 25 35 73 56 21\\n\\n<span class="prompt">kali@kali:~/Desktop$</span> <span class="cmd">python3 -c "print(\\'RandCrack successful\\')"</span>\\n<span style="color:#3ddc84; font-weight:bold;">[+] Predict next random number: 54</span>';
    }
  }, 1000);
}
</script>"""
...
db.session.commit()
print("Lesson 172 HTML Entities fixed and Live Flow simulation fully operational!")
ctx.pop()
